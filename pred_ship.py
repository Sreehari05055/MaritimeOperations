...
new_data_fuel[['CO2_emissions', 'distance']] = scaler_fuel.transform(
    new_data_fuel[['CO2_emissions', 'distance']].values)
...
new_data_dist_pred[['CO2_emissions', 'fuel_consumption']] = scaler_dist.transform(
    new_data_dist_pred[['CO2_emissions', 'fuel_consumption']].values
)
...
print(f"Fuel Consumption: {new_data['fuel_consumption'][0]} Liters --> Predicted Distance: {predicted_dist[0]:.2f} Km")
print(f"Distance: {new_data['distance'][0]} Km --> Predicted Fuel Consumption: {predicted_fuel[0]:.2f} Liters")
